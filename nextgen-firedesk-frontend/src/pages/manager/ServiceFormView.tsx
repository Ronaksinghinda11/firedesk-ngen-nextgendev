/**
 * Service Form View Page for Managers
 * View submitted service forms with answers and approve/reject them
 */
...
import ServiceFormReviewModal from '@/components/ServiceForms/Manager/ServiceFormReviewModal';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function ServiceFormView() {
  ...
  const fetchServiceFormView = async () => {
    try {
      setLoading(true);
      const response = await managerServiceFormApi.getServiceFormView(serviceId!);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      setPdfLoading(true);
      const { blobUrl } = await managerServiceFormApi.getSubmissionPDF(serviceId);
  ...
  const handleActualDownload = () => {
    if (pdfPreviewUrl && serviceData) {
      ...
      managerServiceFormApi.downloadFromUrl(pdfPreviewUrl, fileName);
      toast({
        title: 'Success',
        ...
        <div className="max-w-7xl mx-auto">
          <Card>
            <CardContent className="pt-6 text-center">
              ...
              <Button onClick={() => navigate('/manager/calendar')}>
                Back to Calendar
              </Button>
            </CardContent>
          </Card>
  ...
  return (
    <>
              ...
              <CardHeader className="relative">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-orange-50/90 text-base mt-2">
              ...
              <CardContent className="relative">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                  ...
                  <div className="flex items-center gap-3 bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
                    ...
                    <div>
                      <p className="text-xs text-orange-50/70 uppercase tracking-wide font-medium">
                        Asset ID
                      </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  {serviceData.submittedBy && (
                    <div className="flex items-center gap-2 text-white/90 text-sm bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20">
                      ...
                      <span>By: {serviceData.submittedBy.name || 'N/A'}</span>
                    </div>
                  )}
            ...
            <Card className="mb-6">
              <CardContent className="pt-6">
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="text-center">
                    <p className="text-3xl font-bold text-blue-600">{statistics.questionsWithPhotos}</p>
                    <p className="text-sm text-gray-600 mt-1">With Photos</p>
                  </div>
                  <div className="text-center">
                    <p className="text-3xl font-bold text-green-600">{statistics.questionsWithNotes}</p>
            ...
            {!hasAnswers && (
              <div className="px-6 py-4 bg-blue-50 border-b border-blue-200">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-medium text-blue-900">Service Not Yet Submitted</p>
                    ...
                    {formData.sections.map((section: any, sectionIndex: number) => (
                      <Fragment key={section.id}>
                        ...
                        <tr className="bg-gradient-to-r from-orange-50 to-orange-100/30">
                          <td colSpan={5} className="px-6 py-3 font-semibold text-gray-800 text-base border-y border-orange-200">
                            {section.sectionName}
                            {section.description && (
      ...
      {/* Review Modal */}
      <ServiceFormReviewModal
        open={showReviewModal}
        onOpenChange={setShowReviewModal}
        serviceId={serviceId!}
        serviceName={formData.serviceName}
        submissionNumber={serviceData.submissionNumber}
        onSuccess={fetchServiceFormView}
      />
    </>
  );
}
