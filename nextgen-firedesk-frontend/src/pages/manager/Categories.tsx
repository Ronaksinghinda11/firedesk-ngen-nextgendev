// Manager Categories Page - Full CRUD with Wizard
// Managers with permissions can create, edit, and delete categories just like admins
...
// Tag Input Component for ITM Parameters
const TagInput = ({
  tags,
  onChange,
  suggestions = [],
    ...
    <div className="space-y-1">
      {Array.isArray(tags) && tags.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {tags.map((tag, index) => (
            <div key={index} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 bg-blue-100 text-blue-800 rounded text-xs">
              <span>{tag}</span>
      ...
      <div className="relative">
        ...
        {isOpen && filteredSuggestions.length > 0 && (
          <div ref={dropdownRef} className="absolute z-50 w-full mt-1 bg-white border rounded-md shadow-lg max-h-40 overflow-auto">
            {filteredSuggestions.slice(0, 10).map((suggestion, index) => (
              <div key={index} className="px-2 py-1.5 cursor-pointer hover:bg-gray-100 text-sm" onClick={() => handleSuggestionClick(suggestion)}>
                {suggestion}
              </div>
            ))}
          </div>
        ...
        {inputValue.trim() !== '' && !suggestions.includes(inputValue.trim()) && (
          <p className="text-[10px] text-blue-600 mt-0.5">Press Enter to add "{inputValue}"</p>
        )}
      </div>
    </div>
  );
        ...
        return (
          <div className="space-y-3 max-w-3xl">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="space-y-1">
                <Label htmlFor="isFireExtinguisher" className="text-xs font-medium text-gray-700">
                  Is Fire Extinguisher?
                </Label>
                <Select
                  value={formData.isFireExtinguisher === true ? 'Yes' : 'No'}
                  onValueChange={(value) => {
                    setFormData({ ...formData, isFireExtinguisher: value === 'Yes' });
                  }}
                >
                  <SelectTrigger className="h-9 text-sm">
                    <SelectValue placeholder="Select option" />
  ...
  const onWizardSubmit = async (formData: any) => {
    try {
      ...
      const categoryData: any = {
        ...
        is_fire_extinguisher: formData.isFireExtinguisher === true,
      ...
      // Handle spec definitions with SMART SYNC (preserves asset values)
      ...
      toast({
        ...
        description: `${specifications.length} specification(s) have been saved.`,
      });
  ...
  const categoryConfig: EntityConfig = {
      ...
      if (response && Array.isArray(response.allCategory)) {
        const transformedData = response.allCategory.map(
          (item: any) => ({
            ...
            isFireExtinguisher: item.is_fire_extinguisher || false,
    ...
    transformData: (data: any) => {
      const transformed = {
        ...
        is_fire_extinguisher: data.isFireExtinguisher === true,
      ...
      try {
        ...
        if (category.specDefinitions && Array.isArray(category.specDefinitions)) {
          ...
          try {
            ...
            console.log("📥 Loaded specifications with itmParameters:", specifications);
          } catch (error) {
        ...
        return {
          ...
          formId: category.form_id || category.formId,
          testFrequencyRequired: category.test_frequency_required || category.testFrequencyRequired || false,
          isFireExtinguisher: category.is_fire_extinguisher || category.isFireExtinguisher || false,
...
const SpecificationsManager: React.FC<SpecificationsManagerProps> = ({
  ...
  useEffect(() => {
    const fetchItmParameters = async () => {
      try {
        ...
        console.warn('Could not fetch ITM parameters:', error);
      }
    };
    fetchItmParameters();
  }, []);
  ...
  const addSpecification = () => {
    ...
    setNewSpec({ label: "", fieldType: "text", isRequired: false, order: updatedSpecs.length, options: [], itmParameters: [] });
  };

  const editSpecification = (index: number) => {
    const spec = specifications[index];
    setNewSpec({ label: spec.label, fieldType: spec.fieldType, isRequired: spec.isRequired, order: spec.order, options: spec.options || [], itmParameters: spec.itmParameters || [] });
  ...
  const cancelEditing = () => {
    setEditingIndex(null);
    setNewSpec({ label: "", fieldType: "text", isRequired: false, order: specifications.length, options: [], itmParameters: [] });
  };
    ...
    <div className="space-y-3 max-w-2xl">
      ...
      <div className="border rounded-lg p-2 space-y-2 bg-gray-50">
        <h4 className="font-medium text-xs text-gray-900">
          {editingIndex !== null
            ? "Edit Specification"
            : "Add New ITM Parameter"}
        </h4>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-2 items-end">
          <div className="space-y-0.5">
            <Label className="text-[10px] font-medium">Label <span className="text-red-500">*</span></Label>
            <Input
              type="text"
              value={newSpec.label}
              onChange={(e) =>
