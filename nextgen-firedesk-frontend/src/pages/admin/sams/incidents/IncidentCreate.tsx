...
import { incidentApi, incidentSubtypeApi, IncidentSubtype } from '@/services/api/samsApi';
...
const IncidentCreate = () => {
  ...
  const fetchInitialData = async () => {
    try {
      const [subtypesRes, plantsRes] = await Promise.all([
        ...
        axios.get(`${import.meta.env.VITE_INTERNAL_API_PATH || 'http://localhost:3001'}/plants`, {
  ...
  const fetchBuildings = async (plantId: string) => {
    try {
      ...
      // Ensure IDs are strings and map snake_case to camelCase
      const buildings = (response.data.data || []).map((building: any) => ({
        ...building,
        id: String(building.id),
        buildingName: building.buildingName || building.building_name
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(`${location.pathname.includes('/manager') ? '/manager' : '/admin'}/sams/incidents/${response.data.id}`);
    } catch (error: any) {
      toast({
        title: "Error",
  ...
  if (!canCreate) {
    const basePath = location.pathname.includes('/manager') ? '/manager' : '/admin';
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-200px)]">
        <div className="text-center space-y-4">
          ...
          <Button onClick={() => navigate(`${basePath}/sams/incidents`)}>
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center gap-4">
        <Button
          variant="ghost"
          size="sm"
          onClick={() => navigate(`${location.pathname.includes('/manager') ? '/manager' : '/admin'}/sams/incidents`)}
        >
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          ...
          <CardContent className="space-y-6">
            {/* ... (keeping form content unchanged) */}
            {/* Subtype and Date */}
            ...
            <div className="space-y-2">
              <Label htmlFor="impact">
                Impact of Incident <span className="text-muted-foreground text-xs">(Optional)</span>
              </Label>
              <Textarea
                id="impact"
                value={formData.impact}
                onChange={(e) => handleChange('impact', e.target.value)}
                placeholder="Describe the impact: How many people were affected? What was the severity of consequences? (Optional)"
                rows={4}
                className="resize-none"
              />
              <p className="text-sm text-gray-500">
